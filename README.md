...
| `GroupCheckoutConcurrencyTest`（5） | 揪團的四條金流路徑（團員結帳、團長結帳、補款、取消退款）重複觸發時，同一筆金額只能發生一次，且帳本與餘額必須相符 |
| `DemoApplicationTests`（1） | Spring context 載入 |

`ItemSpecResolverTest` / `ItemHashTest` / `CouponEligibilityTest` 是純邏輯測試，
不載入 Spring context，45 個測試裡它們合計只跑 0.05 秒。
...
| 層 | 內容 | 抓得到什麼 |
|----|------|-----------|
| `mvn test`（45） | 授權、金流與品項規則的回歸防線 | 邏輯錯誤、併發重複扣款與遺失更新、規格與折扣算錯 |
